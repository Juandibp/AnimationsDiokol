local a, CenterX, CenterY, Radius;
...
function noise( x,  y,  z) 
  ...
  function draw()
    ...
    for i = 0, 30 do
      -- update x and y
      local newX = cy + a*(1-b*math.pow(cy,2))*cy + m*cx + 2*(1-m)*math.pow(cx,2) / (1 + math.pow(cx,2));
      local newY = -cx + m*newX + 2*(1-m)*math.pow(newX,2) / (1 + math.pow(newX,2));
    
      -- calculate the distance between the new point and the center
      local distance = dist((width() / 2 + newX/s), (height() / 2 + newY/s), (width()/2), height()/2);
    
      -- color accoriding to the distance
      local n = noise((t)/30,0,0);
      ...
      ellipse((width() / 2 + newX/s), (height() / 2 + newY/s), d, d);
      end
...
function draw() 
  ...
  for i = 0,180
  do
    local x = CenterX + Radius*math.cos(TWO_PI/180 * i);
    local y = CenterY + Radius*math.sin(TWO_PI/180 * i);
    a = 150 * noise(a*10,0,0);
    local rl = Radius * 3 + a * map(uistate.mousex, 0, width(), -1.5, 2.75);
    a = a + 0.03;

    local rx = x + rl * math.cos(-PI/2.5 + TWO_PI/180 * i);
    local ry = y + rl * math.sin(-PI/2.5 + TWO_PI/180 * i);
    strokeWeight(1);
    line(x, y, rx, ry);
  end
  ...
  stroke(255);
  fill(0);
  ellipse(CenterX, CenterY, 2*Radius+20, 2*Radius+20);
end